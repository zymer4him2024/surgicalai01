...
| DATA INFO Panel (FROZEN) | HDMI HUD shows active preset targets (instrument name + required count) in DATA INFO box above TRAY INFO. Same width as TRAY INFO (310px), 50% taller vertically (row_h=39). Positioned dynamically above TRAY INFO with 10px gap. Gateway idle loop re-sends `scan_info` every ~5s so display recovers after restart. **Do not modify HUD layout logic without explicit request.** |
| TRAY INFO Persistence (FROZEN) | TRAY INFO panel persists previous detection data until next non-empty update arrives (`_last_tray_items` cache in HUDRenderer). Panel height is responsive to item count, anchored from bottom of screen. **Do not modify HUD layout logic without explicit request.** |
...
27. **DATA INFO not showing after display restart**: Gateway sends `scan_info` only on state transitions (once). After display restart, `scan_info` is `None` in DisplayState. Fix: gateway idle loop re-sends `scan_info` from `_pending_preset` every ~5s (`_idle_hud_tick` counter).
28. **TRAY INFO disappearing between inference cycles**: Gateway sends `"tray_items": []` on state transitions and when inference returns no detections. Fix: `HUDRenderer._last_tray_items` cache persists previous data; only overwritten when new non-empty data arrives.
